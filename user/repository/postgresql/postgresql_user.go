...
type postgresqlUserFilepathRepository struct {
	Conn *gorm.DB
}

func NewMysqlUserRepository(Conn *gorm.DB, Casbin *casbin.Enforcer) domain.UserRepository {
	return &postgresqlUserRepository{Conn, Casbin}
}
func NewMysqlUserFilepathRepository(Conn *gorm.DB) domain.UserFilepathRepository {
	return &postgresqlUserFilepathRepository{Conn}
}

func (m *postgresqlUserFilepathRepository) Store(ctx context.Context, u *domain.UserFilepath) (err error) {
	log.Println("HEEREEE")
...
func (m *postgresqlUserRepository) Store(ctx context.Context, u *domain.User) (err error) {
...
func (m *postgresqlUserRepository) Update(ctx context.Context, u *domain.User) (err error) {
...
func (m *postgresqlUserRepository) GetByID(ctx context.Context, id uuid.UUID) (res domain.User, err error) {
...
func (m *postgresqlUserRepository) GetOneByUsernameOrEmail(ctx context.Context, usernameOrEmail string) (res domain.User, err error) {
	conn := m.Conn.WithContext(ctx)
	var user []domain.User
...
func (m *postgresqlUserRepository) GetOne(ctx context.Context, args domain.QueryArgs) (res domain.User, err error) {
	var user domain.User
