...
// @Accept       mpfd
// @Produce      json
// @Param		 account formData domain.Auth true "login account"
...
func (a *AuthHandler) Login(c echo.Context) (err error) {
	...
	cookie := &http.Cookie{
		Name:     "jwt_token",
		Value:    data.Token,
		Expires:  time.Now().Add(time.Hour * 24), // Set expiration time as needed
		HttpOnly: true,
	}
	c.SetCookie(cookie)
	response := responses.CustomResponse(data, http.StatusOK, true, "operation success")
...
func (a *AuthHandler) Logout(c echo.Context) error {
	// Clear the JWT token cookie to log out
	cookie := &http.Cookie{
		Name:     "jwt_token",
		Value:    "",
		Expires:  time.Now(),
	...
	return c.JSON(http.StatusOK, responses.CustomResponse(nil, http.StatusOK, true, "success logout"))
}
